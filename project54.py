...
def evaluate_card(card_str):
    card_value = CardType.TWO
    card_color = CardColor.SPADE

    if card_str[0] == '2':
        card_value = CardType.TWO
    elif card_str[0] == '3':
        card_value = CardType.THREE
    elif card_str[0] == '4':
        card_value = CardType.FOUR
    elif card_str[0] == '5':
        card_value = CardType.FIVE
    elif card_str[0] == '6':
        card_value = CardType.SIX
    elif card_str[0] == '7':
        card_value = CardType.SEVEN
    elif card_str[0] == '8':
        card_value = CardType.EIGHT        
    elif card_str[0] == '9':
        card_value = CardType.NINE
    elif card_str[0] == 'T':
        card_value = CardType.TEN
    elif card_str[0] == 'J':
        card_value = CardType.JACK
    elif card_str[0] == 'Q':
        card_value = CardType.QUEEN
    elif card_str[0] == 'K':
        card_value = CardType.KING
    elif card_str[0] == 'A':
        card_value = CardType.AS
    
    if card_str[1] == 'S':
        card_color = CardColor.SPADE
    elif card_str[1] == 'H':
        card_color = CardColor.HEART
    elif card_str[1] == 'D':
        card_color = CardColor.DIAMOND
    elif card_str[1] == 'C':
        card_color = CardColor.CLUB
    
    return card_value, card_color

def get_nb_card(n, value):
    res = []
    for i in range(len(value)):
        if value[i] == n:
            res.append(CardType(i))
    return res
...
def get_straight(value):
    ...
    for i in range(len(value)):
        ...
        if value[i] > 1:
            return None
        # Not successive
        if value[i] == 0 and nb_1 > 0 and nb_1 < 5:
            return None

        if value[i] == 1:
            nb_1 += 1
            if start_card == None:
                start_card = CardType(i)
            if nb_1 == 5:
                return start_card
    return None

def evaluate_poker_hand(poker_hand_player):
    color = [ 0 ] * 4
    value = [ 0 ] * 13
    for card_str in poker_hand_player:
        card_value, card_color = evaluate_card(card_str)
        value[card_value.value] += 1
        color[card_color.value] += 1

    flush = False
    if 5 in color:
        flush = True

    straight = get_straight(value)
    four_of_kind = get_nb_card(4, value)
    three_of_kind = get_nb_card(3, value)
    pairs = get_nb_card(2, value)
    high_card = None if get_nb_card(1, value) == [] else max(get_nb_card(1, value))
    
    if flush and straight is not None:
        return HandRanked.STRAIGHT_FLUSH, straight

    if four_of_kind != []:
        return HandRanked.FOUR_OF_A_KIND, four_of_kind[0]

    if three_of_kind != [] and pairs != []:
        return HandRanked.FULL, three_of_kind[0], pairs[0]
    
    if flush:
        return HandRanked.FLUSH,
    
    if straight is not None:
        return HandRanked.STRAIGHT, straight
    
    if three_of_kind != []:
        return HandRanked.THREE_OF_A_KIND, three_of_kind[0]

    if len(pairs) == 2:
        return HandRanked.TWO_PAIRS, max(pairs), min(pairs)
    
    if pairs != []:
        return HandRanked.ONE_PAIR, pairs[0]

    return HandRanked.HIGH_CARD, high_card


def parse_poker_hand(poker_hand):
    split = poker_hand.split(' ')
    hand1 = split[:5]
    hand2 = split[5:]
    return hand1, hand2

def get_winner(poker_hand):
    hand_p1, hand_p2 = parse_poker_hand(poker_hand)
    res_p1 = evaluate_poker_hand(hand_p1)
    res_p2 = evaluate_poker_hand(hand_p2)

    if res_p1[0] > res_p2[0]:
        return 1
    if res_p1[0] < res_p2[0]:
        return 2
    ...
    # Same rank
    len_res = len(res_p1)
    if len_res > 1 and res_p1[1] > res_p2[1]:
        return 1
    if len_res > 1 and res_p1[1] < res_p2[1]:
        return 2

    if len_res > 2 and res_p1[2] > res_p2[2]:
        return 1
    if len_res > 2 and res_p1[2] < res_p2[2]:
        return 2
...
def read_poker_hands():
    p1_win = 0
    for poker_hand in poker_datas:
        winner = get_winner(poker_hand)
        if winner == 1:
            p1_win += 1
    return p1_win


print(read_poker_hands())